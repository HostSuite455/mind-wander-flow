...
interface CleanerPropertyPickerProps {
  cleanerId: string;
  selectedPropertyId?: string;
  onPropertySelect: (propertyId: string) => void;
}

export default function CleanerPropertyPicker({ cleanerId, selectedPropertyId, onPropertySelect }: CleanerPropertyPickerProps) {
  ...
  const loadProperties = async () => {
    try {
      ...
      const propertiesList: Property[] = rawData
        .map(a => a.properties)
        .filter((p: any): p is any => p !== null && p !== undefined)
        .map((p: any) => ({
          id: p.id as string,
      ...
      if (propertiesList.length === 1 && !selectedPropertyId) {
